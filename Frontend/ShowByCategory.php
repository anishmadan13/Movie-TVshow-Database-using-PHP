...
<div class="container_wrapper">
    ...
    <div class="col2">
    	<div class="search_box">
        	<form name="search" id="search" method="post" action="SearchShow.php" onsubmit="return validate()">
            	<table>
                	<tr>
                    	<td>Category</td>
                    	<td>
                        ...
                        <td>
                        	<input type="text" id="search" name="search" placeholder="Enter Show Name" size="39"/>
                        </td>
                        <td>
     	...
     	<div id="header_title">Shows List</div>
        <?php
		error_reporting(E_ERROR);
		$id = $_GET['id'];
		$line = 0;
		$page = 'Category.php';
		$dataperpage = mysqli_query($connect,"SELECT * FROM tblshows WHERE id = '$id'");
        ...
        $getShow = mysqli_query($connect,"SELECT * FROM tblshows  WHERE id = '$id' ORDER BY id LIMIT $eu,$limit");
        while($rowShow = mysqli_fetch_array($getShow)){
       	?>	
     		<div class="content_holder">
            	<div class="info">
                	<?php echo "<img src=../Administrator/PHP/upload_images/shows/$rowShow[showimage] height=70 width=70/>"; ?> 
                	<div class="info1">
                    	<?php
						$show = strtoupper($rowShow['showname']);
						echo '<table cellpadding=0 cellspacing=0>';
							echo '<tr>';
								echo '<td><label id=show>Series Name:</label></td>';
								echo "<td><a href='ViewShow.php?id=".$rowShow['showname']."' id=link>".$show."</td>";
							echo '</tr>';
							echo '<tr>';
								echo '<td><label id=a1>Director:</label></td>';
								echo '<td><label id=a2>'.$rowShow['showdir'].'</label></td>';
							echo '</tr>';
							echo '<tr>';
								echo '<td><label id=a1>Genre:</label></td>';
								echo '<td><label id=a2>'.$rowShow['showcat'].'</label></td>';
							echo '</tr>';
						echo '</table>';
						?>
                    </div>
