...
<div class="container_wrapper">
    ...
    <div class="col2">
		...
		$search = $_POST['search'];
		$searchquery = mysqli_query($connect,"SELECT * FROM tblshows WHERE 
							showname LIKE '%$search%' ORDER BY id LIMIT 10");
		while($rowSearch = mysqli_fetch_array($searchquery)){
       	?>	
     		<div class="content_holder">
            	<div class="info">
                	<?php echo "<img src=../Administrator/PHP/upload_images/shows/$rowSearch[showimage] height=70 width=70/>"; ?> 
                	<div class="info1">
                    	<?php
						$show = strtoupper($rowSearch['showname']);
						echo '<table cellpadding=0 cellspacing=0>';
							echo '<tr>';
								echo '<td><label id=show>Series Name:</label></td>';
								echo "<td><a href='ViewShow.php?id=".$rowSearch['showname']."' id=link>".$show."</td>";
							echo '</tr>';
							echo '<tr>';
								echo '<td><label id=a1>Director:</label></td>';
								echo '<td><label id=a2>'.$rowSearch['showdir'].'</label></td>';
							echo '</tr>';
							echo '<tr>';
								echo '<td><label id=a1>Genre:</label></td>';
								echo '<td><label id=a2>'.$rowSearch['showcat'].'</label></td>';
							echo '</tr>';
						echo '</table>';
						?>
                    </div>
